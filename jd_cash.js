...
!(async () => {
  ...
  await requireConfig()
  $.authorCode = await getAuthorShareCode('https://raw.gi1thubusercontent.com/Aaron-lv/updateTeam/master/shareCodes/jd_updateCash.json')
  if (!$.authorCode) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/Aa1ron-lv/updateTeam@master/shareCodes/jd_updateCash.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorCode = await getAuthorShareCode('https://cd1.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_updateCash.json') || []
  }
