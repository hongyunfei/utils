...
async function accountCheck() {
  ...
  let client = new WebSocket(`wss://xinruimz-isv.isvjcloud.com/wss/?token=${$.token}`, null, {
    headers: {
      'user-agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "Dalvik/2.1.0 (Linux; U; Android 10; TAS-AN00 Build/HUAWEITAS-AN00)"),
    }
  });
  client.onopen = async () => {
  ...
  client.onmessage = async function (e) {
    if (e.data !== 'pong' && e.data && safeGet(e.data)) {
      ...
      switch (vo.action) {
        ...
        //       if (parseInt(benefit.setting.beans_count) === bean500 && //兑换多少豆 bean500就500豆
         //        $.total > benefit.coins &&
