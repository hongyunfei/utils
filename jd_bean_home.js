...
!(async () => {
  $.newShareCodes = []
  $.authorCode = await getAuthorShareCode('https://raw.gi1thubusercontent.com/Aaron-lv/updateTeam/master/shareCodes/jd_updateBeanHome.json')
  if (!$.authorCode) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/A1aron-lv/updateTeam@master/shareCodes/jd_updateBeanHome.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorCode = await getAuthorShareCode('https://cd1n.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_updateBeanHome.json') || []
  }
  if (!cookiesArr[0]) {
